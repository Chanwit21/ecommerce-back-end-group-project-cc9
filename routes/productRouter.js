...
productRouter.delete('/:productId', passport.authenticate('jwtAdmin', { session: false }), productController.deleteProduct)
productRouter.post('/', passport.authenticate('jwtAdmin', { session: false }), upload.array('imageUrl', 7), productController.createNewProduct)
productRouter.get('/productImage/:productId', passport.authenticate('jwtAdmin', { session: false }), productController.getProductImageByProductId)
productRouter.put('/:productId', passport.authenticate('jwtAdmin', { session: false }), upload.array('imageUrl', 7), productController.updateProduct)
productRouter.get('/readyToShip/:productId', passport.authenticate('jwtAdmin', { session: false }), productController.readyToShip)
