...
exports.getOrderByTime = async (req, res, next) => {
  try {
    ...
    if (selectedTime === 'month') {
      ...
      const weeksInYear = moment(startTime).day(-7 * (oldWeeksInMonth - 1)).week()
      console.log(`start`, moment.utc().week(weeksInYear).day(0).hour(0).minute(0).second(0))
      console.log(`end`, moment.utc().week(weeksInYear).day(7 * (oldWeeksInMonth - 1)).hour(0).minute(0).second(0))
      oldOrder = await Order.findAll({
        where: {
          status: 'successful',
          paidAt: {
            [Op.gte]: moment.utc().week(weeksInYear).day(0).hour(0).minute(0).second(0),
            [Op.lt]: moment.utc().week(weeksInYear).day(7 * (oldWeeksInMonth - 1)).hour(0).minute(0).second(0)
          }
        }
      })
    }
